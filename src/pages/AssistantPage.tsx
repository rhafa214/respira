...
export default function AssistantPage() {
  ...
  const [messages, setMessages] = useState<
    { role: "model" | "user"; text: string }[]
  >(() => {
    const saved = localStorage.getItem(
      `assistant_chat_history_${user?.uid || "guest"}`,
    );
    if (saved) {
      try {
    ...
    return [
      {
        role: "model",
        text: "Olá! Sou sua assistente financeira pessoal. Como posso te ajudar a organizar suas contas ou revisar seu mês?",
      },
    ];
  });
  ...
  useEffect(() => {
    if (user) {
      localStorage.setItem(
        `assistant_chat_history_${user.uid}`,
  ...
  const handleSend = async (audioBase64?: string, textParam?: string) => {
    ...
    try {
      ...
      const marketBudgetSnap = await getDoc(
        doc(db, "market_budgets", currentMonthStr),
      );
      ...
      const marketItems = marketSnap.docs.map((d) => ({
        id: d.id,
        ...(d.data() as any),
      }));
      const marketBudget = marketBudgetSnap.exists()
        ? marketBudgetSnap.data().amount
        : null;
      ...
      contextStr += `Transações recentes (${transactions.length}):\n${transactions
        ...
        .map(
          (t) =>
            `- ID: ${t.id} | Descrição: ${t.description} | R$ ${t.amount} em ${t.category} (${t.type}) dia ${t.date} ${t.installmentInfo ? "| Parcela: " + t.installmentInfo : ""}`,
        )
        .join("\n")}\n`;
      ...
      const addTransactionDeclaration: FunctionDeclaration = {
        name: "addTransaction",
        description:
          "Adiciona uma nova transação (gasto ou ganho) ao banco de dados financeiro do usuário.",
        parameters: {
          type: Type.OBJECT,
          properties: {
            amount: {
              type: Type.NUMBER,
              description: "O valor da transação em Reais (positivo, ex: 54).",
            },
            category: {
              type: Type.STRING,
              description:
                "A categoria (ex: 'Mercado', 'Gasolina', 'Salário', 'Lazer').",
            },
            type: {
              type: Type.STRING,
              description: "'expense' para gastos ou 'income' para ganhos.",
            },
            date: {
              type: Type.STRING,
              description:
                "A data no formato 'YYYY-MM-DDT12:00:00Z'. Tente deduzir a data correta para 'hoje'.",
            },
            description: {
              type: Type.STRING,
            ...
            installmentInfo: {
              type: Type.STRING,
              description:
                "Informação sobre parcelas correspondentes ao mês dessa mesma transação, ex: '8/36'",
            },
          },
        ...
        parameters: {
          ...
          properties: {
            transactionId: {
              type: Type.STRING,
              description:
                "O ID da transação a ser atualizada (você deve procurar o ID na lista de Transações recentes).",
            },
            amount: {
              type: Type.NUMBER,
              description: "O novo valor da transação, se houver alteração.",
            },
            ...
            installmentInfo: {
              type: Type.STRING,
              description: "A nova informação de parcela ex: '8/36'.",
            },
            status: {
              type: Type.STRING,
              description: "Status pago ('paid') ou pendente ('pending')",
            },
      ...
      const addMarketItemDeclaration: FunctionDeclaration = {
        name: "addMarketItem",
        description:
          "Adiciona um novo item à lista de compras de mercado do mês atual.",
        parameters: {
          type: Type.OBJECT,
          properties: {
            name: {
      ...
      const updateMarketItemDeclaration: FunctionDeclaration = {
        ...
        description:
          "Atualiza um item da lista de compras (preço real ou comprado).",
        parameters: {
          type: Type.OBJECT,
          properties: {
            itemId: {
            ...
            purchased: {
              type: Type.BOOLEAN,
              description: "Se o item foi comprado / colocado no carrinho",
            },
          },
          required: ["itemId"],
        },
      ...
      const aiResponse = await ai.models.generateContent({
        ...
        config: {
          systemInstruction:
            "Você é o 'Consultor IA', especialista financeiro. Quando o usuário perguntar 'E se...' ou usar o Simulador/Máquina do Tempo, faça simulações matemáticas precisas baseadas na realidade do usuário, mostrando claramente os ganhos, perdas, projeções de meses e saldo final para ajudar em grandes decisões. Você pode responder perguntas e ajudar o usuário a registrar ou atualizar transações com as funções fornecidas. Se ele pedir para atualizar ou editar algo, localize o ID na lista e use updateTransaction. Pode também atualizar itens na sua lista de mercado. Não mencione o ID textualmente na resposta, apenas diga que alterou de forma amigável.",
          temperature: 0.7,
          tools: [
            {
              functionDeclarations: [
                ...
                addMarketItemDeclaration,
                updateMarketItemDeclaration,
              ],
            },
          ],
        },
        ...
        for (const call of functionCalls) {
          ...
          if (call.name === "updateTransaction") {
            ...
            try {
              ...
              if (args.installmentInfo !== undefined)
                updateData.installmentInfo = args.installmentInfo;
              if (args.status !== undefined) updateData.status = args.status;
              if (args.date !== undefined) updateData.date = args.date;

              await updateDoc(
                doc(db, "transactions", args.transactionId),
                updateData,
              );
              updatedCount++;
            } catch (err) {
          ...
          if (call.name === "updateMarketItem") {
            ...
            try {
              ...
              if (args.purchased !== undefined)
                updateData.purchased = args.purchased;

              await updateDoc(
                doc(db, `market_items_${currentMonthStr}`, args.itemId),
                updateData,
              );
              updatedMarketCount++;
            } catch (err) {
        ...
        if (!finalMessage) {
          ...
          if (updatedCount > 0)
            finalMessage = finalMessage
              ? finalMessage +
                ` Ah, e atualizei ${updatedCount > 1 ? "as informações que você pediu" : "a informação que você pediu"} também.`
              : `Pronto! Atualizei a transação pra você.`;
          if (addedMarketCount > 0)
            finalMessage = `Adicionei ${addedMarketCount} itens na sua Lista de Mercado daquele mês!`;
          if (updatedMarketCount > 0)
            finalMessage = `Atualizei ${updatedMarketCount} itens no seu mercado.`;
        }
      }
  ...
  const startRecording = async () => {
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "Não foi possível acessar seu microfone. Verifique as permissões.",
        },
      ]);
    }
  };
  ...
  const startLiveSession = async () => {
    ...
    try {
      const contextStr = encodeURIComponent(
        user ? `Nome: ${user.displayName || "Vitor"}` : "",
      );
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "🎙️ (Modo Conversa Ativo) Olá! Estou te ouvindo, o que manda?",
        },
      ]);
    } catch (e) {
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "Não foi possível iniciar a conversa. Verifique o microfone.",
        },
      ]);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 flex flex-col h-[calc(100vh-80px)] md:h-screen">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Tire dúvidas, peça conselhos ou monte estratégias
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="outline"
            className="gap-2 rounded-full hidden sm:flex text-slate-500 border-slate-200 hover:bg-slate-100"
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        <div className="mb-4">
          <h2 className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide flex items-center gap-2">
            <Target className="w-4 h-4" />
            Ideias de Simulações e Análises
          </h2>
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => {
                setInput(
                  "E se eu reduzir meus gastos variáveis em R$ 300 este mês? Como fica minha projeção e margem livre?",
                );
              }}
              className="px-4 py-2 bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl text-sm font-medium text-slate-700 transition-colors shadow-sm"
            >
              Simular: Reduzir Gastos 📉
            </button>
            <button
              onClick={() => {
                setInput(
                  "Máquina do Tempo: E se eu pegar um empréstimo de R$ 5.000 com taxa de 2.5% ao invés de usar minha reserva?",
                );
              }}
              className="px-4 py-2 bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl text-sm font-medium text-slate-700 transition-colors shadow-sm"
            >
              Simular: Empréstimo 🕰️
            </button>
            <button
              onClick={() => {
                setInput(
                  "Quais categorias estou gastando mais que o ideal baseado no meu histórico?",
                );
              }}
              className="px-4 py-2 bg-white border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50 rounded-xl text-sm font-medium text-slate-700 transition-colors shadow-sm"
            >
        ...
        <Card className="flex-1 flex flex-col min-h-0 relative shadow-sm border border-slate-100">
          {isLiveActive && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center pointer-events-none rounded-xl">
              ...
              <p className="mt-8 text-indigo-800 font-medium text-lg tracking-wide z-20 animate-pulse">
                Consultor está ouvindo você...
              </p>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
            {messages.map((m, idx) => (
              ...
              >
                <div
                  className={`w-8 h-8 sm:w-10 sm:h-10 shrink-0 rounded-full flex items-center justify-center ${m.role === "model" ? "bg-indigo-100 text-indigo-600" : "bg-emerald-100 text-emerald-600"}`}
                >
                  {m.role === "model" ? (
                ...
                <div
                  className={`px-5 py-3.5 rounded-2xl max-w-[85%] text-sm sm:text-base ${m.role === "model" ? "bg-slate-50 text-slate-800 rounded-tl-sm" : "bg-emerald-500 text-white rounded-tr-sm"}`}
                >
                  {m.role === "model" ? (
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        className="self-end text-slate-400 hover:text-indigo-600 transition-colors p-1"
            ...
            >
              ...
              <Input
                ...
                placeholder={
                  isRecording
                    ? "Gravando áudio..."
                    : "Pergunte sobre seus limites, como organizar suas contas..."
                }
                value={input}
              ...
              <Button
                type="submit"
                size="icon"
                className="h-12 w-12 rounded-full shrink-0 bg-indigo-600 hover:bg-indigo-700"
