...
export function GlobalChatFAB() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <>
          <div className="fixed inset-0 z-40" onClick={toggleChat} />
          <ChatPanel />
        </>
      )}
    </>
    ...
    <div className="fixed bottom-24 md:bottom-24 right-4 md:right-8 w-[calc(100vw-32px)] sm:w-[400px] h-[500px] max-h-[calc(100vh-140px)] bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col z-50 overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300">
      ...
      <div className="bg-indigo-600 p-4 text-white flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-200">
              Online e conectado às suas contas
            </p>
          </div>
        </div>
        <div className="flex gap-1 items-center">
          <Button
            ...
            className="text-white hover:bg-white/20 hover:text-white rounded-xl"
          ...
          <Button
            ...
            className={`rounded-xl hover:bg-white/20 hover:text-white ${isLiveActive ? "bg-rose-500 hover:bg-rose-600 text-white" : "text-white"}`}
          >
            {isLiveActive ? (
      ...
      {isLiveActive && (
        <div className="absolute inset-y-0 inset-x-0 mt-[72px] bg-white/90 backdrop-blur z-10 flex flex-col items-center justify-center pointer-events-none rounded-b-3xl">
          ...
          <p className="mt-8 text-indigo-800 font-medium tracking-wide z-20 animate-pulse">
            Ouvindo você...
          </p>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
        {messages.map((m, idx) => (
          ...
          >
            <div
              className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center text-white ${m.role === "model" ? "bg-indigo-600" : "bg-emerald-500"}`}
            >
              {m.role === "model" ? (
      ...
      <div className="p-3 bg-white border-t border-slate-100 shrink-0">
        ...
        >
          ...
          <Input
            ...
            placeholder={
              isRecording ? "Gravando áudio..." : "Diga 'Paguei o aluguel'..."
            }
            value={input}
